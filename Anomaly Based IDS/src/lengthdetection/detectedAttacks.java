...
public class detectedAttacks extends fileManipulate {
	 ...
	 * */
	public detectedAttacks(){
		thresholdVal = 0;
		detectedAttacks = new ArrayList<Double>();
	}
	 ...
	 * */
	public void detectVal(){
		
			//attackURI.add(valuesASGreaterAvg);
			
			for (double entries: getGreaterThanAvg()){ //iterate first dimension of List<ArrayList<Double>
				//for (Double getVal: entries){//iterate 2nd loop of List<ArrayList<Double>
					
					if (entries>fixedThreshold()){ //AS greater than threshold will be stored in detected attacks
						detectedAttacks.add(entries);
					//}
				}
			}
	...
	public ArrayList<Double> getDetectedAttacks(){
		return detectedAttacks;
	}
}
